...
- Harvested so far, one dir per harvest model under `data/acts/`: `Qwen3.6-27B/` holds 299 rollouts (seed 0: 150 cheated, 149 clean; the one 112k-token clean rollout OOMed on the remote card and is absent), all 64 layers, 16,696 positions, 11 GB. Every harvest is off-policy: the cheating transcripts were generated by qwen3.5/3.8-27B, none by qwen3.6. `Qwen3.8-27B/` holds the same 299 rollouts; qwen3.8 wrote 101 of its 150 cheating and 61 of its 149 clean transcripts.
- `make_vectors.py`: CPU. Difference-of-means directions from the harvest, one `[n_layers, d_model]` tensor per contrast per model under `data/vectors/<model>/` (tracked in git) with a json sidecar of class sizes and per-layer gaps and norms. All vectors use the last content token of each turn's reasoning span. The module docstring records the class definitions, the salience confound that motivates the `cheat_vs_declined` contrast, and the positions that were checked and rejected.
